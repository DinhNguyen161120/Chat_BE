...
const updateWatchedMessageStatusInReduxStore = (listMessage, conversationId) => {
    try {
        ...
        if (check) {
            ...
            const io = socketStore.getInstantSocket();
            io.to(socketId).emit("update-watched-status-message-in-redux-store", {
                listMessage,
                conversationId,
            });
        }
...
const updateSentMessageStatusInReduxStore = (listMessage, conversationId) => {
    try {
        ...
        if (check) {
            const socketId = socketStore.getSocketIdFromUserId(senderId);
            const io = socketStore.getInstantSocket();
            io.to(socketId).emit("update-sent-status-message-in-redux-store", {
                listMessage,
                conversationId,
            });
        }
...
const updateReceivedMessageStatusInReduxStore = (listMessage, conversationId) => {
    try {
        ...
        if (check) {
            const socketId = socketStore.getSocketIdFromUserId(senderId);
            const io = socketStore.getInstantSocket();
            io.to(socketId).emit("update-received-status-message-in-redux-store", {
                listMessage,
                conversationId,
            });
        }
...
module.exports = {
    updateConversation,
    updateWatchedMessageStatusInReduxStore,
    updateSentMessageStatusInReduxStore,
